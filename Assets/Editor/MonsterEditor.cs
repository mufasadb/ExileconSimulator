...
public class MonsterEditor : EditorWindow
{
    ...
    MonsterDataObject singleMonster;
    MonsterDataSet data;
    int defencePhysical;
    int defenceLife;
    int defenceArmour;
    ...
    {
        GetWindow<StaffEditor>("Monster Editor");
    }
    void OnGUI()
    {
        LoadFromJson();
        ...
        if (showForm)
        {
            singleMonster.name = EditorGUILayout.TextField("name", singleMonster.name);
            // singleMonster.type = (Type)EditorGUILayout.EnumFlagsField("Type", singleMonster.type);
            singleMonster.tier = EditorGUILayout.IntField("Tier", singleMonster.tier);
        ...
        else
        {
            ...
            if (GUILayout.Button("New")) { singleMonster = new MonsterDataObject(); showForm = true; ClearStats(); newCard = true; }
        }
    }
    private void Cancel()
    {
        showForm = false; singleMonster = null;
    }
    ...
    {
        singleMonster = data.set[i];
        ClearStats();
        StringToStats(singleMonster.attack, true);
        StringToStats(singleMonster.defence, false);
    }
    ...
    {
        singleMonster.attack = StatsToString(false);
        singleMonster.defence = StatsToString(true);
        if (newCard)
        {
            Debug.Log("added new card");
            data.set.Add(singleMonster);
        }
        else
        {
            data.set[editingInt] = singleMonster;
        }
        Cancel();
    }
    public void LoadFromJson()
    {
        string jsonData = System.IO.File.ReadAllText(Application.dataPath + "/map/monsterData/monsterData.json");
        data = JsonUtility.FromJson<MonsterDataSet>(jsonData);
        // data.set.Sort((c1, c2) => c1.type.CompareTo(c2.type));
        // data.set.Sort((c1, c2) => c1.tier.CompareTo(c2.tier));
    ...
    public void SaveIntoJson()
    {
        ...
        System.IO.File.WriteAllText(Application.dataPath + "/map/monsterData/monsterData.json", saveData);
    }
